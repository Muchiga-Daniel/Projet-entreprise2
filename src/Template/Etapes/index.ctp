...
<div class="etapes index large-12 medium-8 columns content">
    <h3><?= __('Etapes') ?></h3>
    <table cellpadding="0" cellspacing="0">
        ...
        <tbody>
            ...
            <tr>
                <td><?= $this->Number->format($etape->id) ?></td>
                <td><?= h($etape->created) ?></td>
                <td><?= h($etape->modified) ?></td>
                <td><?= h($etape->designation) ?></td>
                <td class="actions">
                    <?= $this->Html->link(__('View'), ['action' => 'view', $etape->id]) ?>
                    <?= $this->Html->link(__('Edit'), ['action' => 'edit', $etape->id]) ?>
                  <!--  <?= $this->Form->postLink(__('Delete'), ['action' => 'delete', $etape->id], ['confirm' => __('Are you sure you want to delete # {0}?', $etape->id)]) ?>-->
                </td>
            </tr>
            <?php endforeach; ?>
        </tbody>
